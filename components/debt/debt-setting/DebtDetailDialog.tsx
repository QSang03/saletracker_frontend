...
export default function DebtDetailDialog({ open, onClose, debtConfigId, onShowAlert }: DebtDetailDialogProps) {
  ...
  useEffect(() => {
    const fetchDebtDetail = async () => {
      ...
      try {
        ...
        const response = await axios.get(`${API_BASE_URL}/debt-configs/${debtConfigId}/detail`, {
        ...
        if (debtData) {
          ...
          if (onShowAlert) {
            onShowAlert({ type: 'error', message: 'Không thể tải chi tiết công nợ - dữ liệu không hợp lệ' });
          }
        }
      } catch (error) {
        ...
        <DialogContent className="!w-[90vw] !max-w-[90vw] !h-[85vh] !max-h-[85vh] !p-0 overflow-hidden">
          <DialogTitle asChild>
            <div className="bg-gradient-to-r from-blue-600 to-blue-800 text-white p-6">
              ...
              <p className="text-blue-100 mt-1">Đang tải thông tin chi tiết...</p>
            </div>
          </DialogTitle>
        ...
        <DialogContent className="!w-[90vw] !max-w-[90vw] !h-[85vh] !max-h-[85vh] !p-0 overflow-hidden">
          <DialogTitle asChild>
            <div className="bg-gradient-to-r from-red-600 to-red-800 text-white p-6">
              <h2 className="text-2xl font-bold">Chi Tiết Công Nợ</h2>
              <p className="text-red-100 mt-1">Không thể tải thông tin chi tiết</p>
            </div>
          </DialogTitle>
          <div className="flex-1 flex items-center justify-center p-6">
            <div className="text-center">
  ...
  const StatusIcon = ({ status }: { status: string }) => {
    ...
    if (status?.toLowerCase().includes("không thành công") || status?.toLowerCase().includes("lỗi"))
      return <span className="inline-block w-3 h-3 rounded-full bg-red-500 mr-2" />;
    if (status?.toLowerCase().includes("chưa gửi"))
      return <span className="inline-block w-3 h-3 rounded-full bg-gray-400 mr-2" />;
    return <span className="inline-block w-3 h-3 rounded-full bg-yellow-400 mr-2" />;
  };
      ...
      <DialogContent className="!w-[85vw] !max-w-[85vw] !h-[80vh] !max-h-[80vh] !p-0 overflow-hidden">
        ...
        <DialogTitle asChild>
          <div className="bg-gradient-to-r from-blue-600 to-blue-800 text-white p-4">
            ...
            <p className="text-blue-100 mt-1 text-sm">Thông tin chi tiết về cấu hình công nợ khách hàng</p>
          </div>
        </DialogTitle>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 p-4">
            ...
            <div className="bg-gray-50 rounded-lg p-4">
              <div className="text-center mb-4">
                <h3 className="font-semibold text-lg text-gray-800 mb-3">Ảnh công nợ</h3>
              ...
              <div className="space-y-2 text-left">
                <div className="bg-white rounded-lg p-3 shadow-sm">
                  <div className="font-semibold text-gray-600 text-sm">Mã khách hàng</div>
                  <div className="text-base font-medium text-gray-800">{debtDetail?.customer_code || '--'}</div>
                </div>
                <div className="bg-white rounded-lg p-3 shadow-sm">
                  <div className="font-semibold text-gray-600 text-sm">Tên khách hàng</div>
                  <div className="text-base font-medium text-gray-800">{debtDetail?.customer_name || '--'}</div>
                </div>
                <div className="bg-white rounded-lg p-3 shadow-sm">
                  <div className="font-semibold text-gray-600 text-sm">Loại khách hàng</div>
                  <div className="text-base font-medium text-gray-800">{getCustomerTypeDisplay(debtDetail?.customer_type || '')}</div>
                </div>
                <div className="bg-white rounded-lg p-3 shadow-sm">
                ...
                <div className="bg-white rounded-lg p-3 shadow-sm">
                  <div className="font-semibold text-gray-600 text-sm">Tình trạng nhắc nợ</div>
                  <div className="flex items-center">
                    <StatusIcon status={debtDetail?.remind_status || ''} />
                    <span className={`text-sm ${getRemindStatusDisplay(debtDetail?.remind_status || '').color}`}>
                      {getRemindStatusDisplay(debtDetail?.remind_status || '').label}
                    </span>
                  </div>
                </div>
                ...
                {debtDetail?.actor && (
                  <div className="bg-white rounded-lg p-3 shadow-sm">
                    <div className="font-semibold text-gray-600 text-sm">Người chỉnh sửa cuối cùng</div>
                    <div className="text-sm font-medium text-green-700">{debtDetail.actor.fullName}</div>
                  </div>
                )}
                {debtDetail?.employee && (
                  <div className="bg-white rounded-lg p-3 shadow-sm">
                    <div className="font-semibold text-gray-600 text-sm">Nhân viên công nợ</div>
                    <div className="text-sm font-medium text-purple-700">{debtDetail.employee.fullName}</div>
                  </div>
                )}
              </div>
            </div>
            ...
            <div className="space-y-4">
              ...
              <div className="bg-yellow-50 rounded-lg p-3 border-l-4 border-yellow-400">
                ...
                <div className="mb-2">
                  <div className="text-xs text-gray-600 mb-1">Thời gian nhắc:</div>
                  <div className="text-sm font-medium text-yellow-700">{debtDetail?.remind_time_1 ? get({ tempField: debtDetail.remind_time_1 }, "tempField", "Chưa nhắc") : "Chưa nhắc"}</div>
                </div>
                <div className="bg-white rounded p-3 text-gray-700 whitespace-pre-line text-sm leading-relaxed shadow-sm">
              ...
              <div className="bg-orange-50 rounded-lg p-3 border-l-4 border-orange-400">
                ...
                <div className="mb-2">
                  <div className="text-xs text-gray-600 mb-1">Thời gian nhắc:</div>
                  <div className="text-sm font-medium text-orange-700">{debtDetail?.remind_time_2 ? get({ tempField: debtDetail.remind_time_2 }, "tempField", "Chưa nhắc") : "Chưa nhắc"}</div>
                </div>
                <div className="bg-white rounded p-3 text-gray-700 whitespace-pre-line text-sm leading-relaxed shadow-sm">
