...
export default function DebtConfigModal({ open, onClose, onSaved }: { open: boolean; onClose: () => void; onSaved?: (result: { success: boolean; message: string }) => void }) {
  ...
  const validationErrors = useMemo(() => {
    ...
    if (remind1 && !remind1Validation.isValid) {
      errors.push({ field: "Câu nhắc nợ lần 1", invalidPatterns: remind1Validation.invalidMatches });
    }
    const remind2Validation = validateBrackets(remind2);
    if (remind2 && !remind2Validation.isValid) {
      errors.push({ field: "Câu nhắc nợ lần 2", invalidPatterns: remind2Validation.invalidMatches });
    }
    const reminderForSaleValidation = validateBrackets(reminderForSale);
    if (reminderForSale && !reminderForSaleValidation.isValid) {
      errors.push({ field: "Câu nhắc cho sale", invalidPatterns: reminderForSaleValidation.invalidMatches });
    }
    return errors;
  ...
  useEffect(() => {
    if (!open) return;
    setLoading(true);
    fetch(`${process.env.NEXT_PUBLIC_API_URL || ""}/system-config/by-section/debt`, {
      headers: {
      ...
      .then((configs) => {
        ...
        const getValue = (name: string) => configs.find((c: any) => c.name === name)?.value || "";
