...
// Removed PaginatedTable import intentionally to control pagination placement
...
export default function DebtHistoryDialog({
  ...
  // Skeleton kept simple but present
  const SkeletonRow = ({ index }: { index: number }) => (
    <TableRow key={`skeleton-${index}`} className="group relative overflow-hidden animate-pulse" style={{ animationDelay: `${index * 100}ms` }}>
  ...
  return (
    <>
      <style jsx>{`
        ...
        @keyframes float { 0%,100% { transform: translateY(0px) rotate(0deg);} 33% { transform: translateY(-8px) rotate(120deg);} 66% { transform: translateY(-4px) rotate(240deg);} }
        @keyframes sparkle { 0%,100% { opacity:0; transform: scale(0) rotate(0deg);} 50% { opacity:1; transform: scale(1) rotate(180deg);} }
        @keyframes quantum { 0%,100% { opacity:0; transform: scale(0) rotate(0deg);} 25% { opacity:0.5; transform: scale(0.5) rotate(90deg);} 50% { opacity:1; transform: scale(1) rotate(180deg);} 75% { opacity:0.5; transform: scale(0.5) rotate(270deg);} }
        ...
        .animate-quantum { animation: quantum 1.5s ease-in-out infinite; }
      ...
      <Dialog open={open} onOpenChange={onClose}>
        <DialogContent className="!w-[50vw] !max-w-[50vw] max-h-[85vh] !h-auto flex flex-col">
          ...
          {/* Main content + footer (footer INSIDE modal) */}
          <div className="flex-1 flex flex-col min-h-0 relative z-10">
            {/* Filters / top controls area (kept similar to original) */}
            <div className="flex items-center gap-4 py-4 z-10">
              <div>
                <select className="border rounded px-3 py-2" value={currentPageSize} onChange={(e) => handlePageSizeChange(Number(e.target.value))}>
            ...
            {/* Pagination footer - INSIDE modal so it never floats outside */}
            <div className="pagination-footer mt-4 px-4 py-3 flex items-center justify-between gap-4 rounded-b">
              ...
              <div className="flex items-center gap-3">
                <button
                  className="px-4 py-2 rounded-full bg-pink-50 text-pink-600 disabled:opacity-50"
