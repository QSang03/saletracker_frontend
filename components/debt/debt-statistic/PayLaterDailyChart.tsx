...
const PayLaterDailyChart: React.FC<PayLaterDailyChartProps> = ({
  ...
  const getGradientForLabel = (label: string, index: number) => {
    const colorMap: { [key: string]: string } = {
      '1-7': 'linear-gradient(135deg, #10B981 0%, #059669 100%)',
      '8-14': 'linear-gradient(135deg, #F59E0B 0%, #D97706 100%)',
      '8-15': 'linear-gradient(135deg, #F59E0B 0%, #D97706 100%)',
      '15-30': 'linear-gradient(135deg, #EF4444 0%, #DC2626 100%)',
      '16-30': 'linear-gradient(135deg, #EF4444 0%, #DC2626 100%)',
  ...
  const customConfig = labels.reduce((acc, label) => {
    acc[label] = { 
      label: label.includes('>') ? `${label} ngày` : `${label} ngày`, 
      ...
      <CardHeader className="relative z-10 pb-6">
        <div className="flex items-start justify-between">
          ...
          <div className="flex items-center gap-2 bg-gradient-to-r from-green-50 to-blue-50 px-4 py-2 rounded-full border border-green-200">
            ...
            <span className="text-sm font-medium text-green-700">Realtime</span>
          </div>
        </div>
      </CardHeader>
            ...
            <RBarChart data={data} margin={{ top: 20, right: 30, left: 20, bottom: 60 }}>
              <defs>
                ...
                {labels.map((label, idx) => (
                  <linearGradient key={`gradient-${label}`} id={`gradient-${label}`} x1="0" y1="0" x2="0" y2="1">
        ...
        <div className="bg-gradient-to-r from-gray-50 to-blue-50 rounded-2xl p-4 border border-gray-100">
          <div className="flex items-center justify-center mb-3">
            <div className="flex items-center gap-2 text-gray-700">
              ...
              <span className="font-semibold text-sm">Khoảng thời gian trễ hẹn</span>
            </div>
          </div>
