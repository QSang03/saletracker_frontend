        ...
        if (msg.type === "qr_code" && msg.data) {
          ...
          switch (msg.message) {
            ...
            case 'LOGIN_STOPPED':
              ...
              break;
            case 'QR_EXPIRED':
              setAlerts([{ type: 'qr_expired', message: 'Mã QR đã hết hạn. Vui lòng nhấn “Liên kết tài khoản Zalo” để lấy mã mới và đảm bảo quét trong thời gian hiệu lực.' }]);
              setLoading(false);
              setQrData(null);
              stopAllConnections();
              break;
            case 'QR_SCANNED':
              setAlerts([{ type: 'qr_scanned', message: `Đã quét mã QR${msg.data?.displayName ? ' bởi ' + msg.data.displayName : ''}. Đang xác thực thông tin, vui lòng chờ trong giây lát...` }]);
              setLoading(true);
              break;
            case 'PROCESSING':
              setAlerts([{ type: 'processing', message: 'Đang xử lý liên kết tài khoản Zalo...' }]);
              setLoading(true);
              break;
            case 'QR_AVAILABLE':
              setAlerts([{ type: 'qr_available', message: 'Mã QR mới đã sẵn sàng. Hãy sử dụng ứng dụng Zalo để quét lại và hoàn tất liên kết.' }]);
              setLoading(false);
              break;
            case 'LOGIN_SUCCESS':
