...
export default function RenameContactModal({ 
  ...
  const save = async () => {
    ...
    try {
      await api.patch(`auto-reply/contacts/${contactId}/rename`, { newName: name.trim() });
      await api.post('auto-reply/zalo/rename-webhook', { contactId, newName: name.trim() });
      onClose();
    } catch (err: any) {
  ...
  return (
    <Dialog open={open} onOpenChange={(v) => !v && onClose()}>
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-hidden bg-gradient-to-br from-green-50/95 via-white/95 to-teal-50/95 backdrop-blur-xl border-0 shadow-2xl">
        ...
        <DialogHeader className="relative pb-6">
          <div className="flex items-center gap-6 mb-4">
            ...
            <div className="flex-1">
              <DialogTitle className="text-3xl font-bold bg-gradient-to-r from-green-600 to-teal-600 bg-clip-text text-transparent mb-2">
                Đổi tên liên hệ
              </DialogTitle>
              <p className="text-gray-500 text-lg leading-relaxed">Cập nhật tên hiển thị cho liên hệ Zalo</p>
            </div>
          </div>
        </DialogHeader>
          ...
          <div className="relative bg-white/80 backdrop-blur-sm border border-white/60 rounded-3xl p-6 shadow-xl">
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <div className="text-xl font-semibold text-gray-800">{currentName}</div>
        ...
        <DialogFooter className="relative pt-6">
