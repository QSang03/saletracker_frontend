...
const AutoGreetingCustomerList: React.FC<AutoGreetingCustomerListProps> = () => {
  ...
  const selectedCustomers = filteredCustomers.filter(customer => selectedCustomerIds.has(customer.id));
  const isAllSelected = currentCustomers.length > 0 && currentCustomers.every(customer => selectedCustomerIds.has(customer.id));
  const isPartiallySelected = currentCustomers.some(customer => selectedCustomerIds.has(customer.id)) && !isAllSelected;
    ...
    try {
      ...
      if (Array.isArray(data) && data.length > 0) {
        console.log("First customer structure:", {
          ...
          zaloDisplayName: data[0].zaloDisplayName,
          salutation: data[0].salutation,
          greetingMessage: data[0].greetingMessage
        });
      }
  ...
  const handleSaveEdit = async () => {
    ...
    try {
      const token = getAccessToken();
      const response = await fetch(`/api/auto-greeting/customers/${editingCustomer.id}`, {
        method: 'PATCH',
        headers: {
  ...
  const handleBulkEditSave = async () => {
    ...
    try {
      ...
      console.log('Bulk update request:', {
        ...
        selectedCustomers: selectedCustomers.map(c => ({ id: c.id, userId: c.userId }))
      });
      ...
      toast.success(`Đã cập nhật ${result.updatedCount || selectedCustomers.length} khách hàng thành công!`, {
        description: 'Thông tin đã được cập nhật cho tất cả khách hàng được chọn',
        duration: 3000,
      });
